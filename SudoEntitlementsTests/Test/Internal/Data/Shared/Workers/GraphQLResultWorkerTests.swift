...
class GraphQLResultWorkerTests: XCTestCase {
    ...
    func test_convertToResult_ReturnsPassedInError() {
        ...
        switch result {
        case let .failure(error as SudoEntitlementsError):
    ...
    func test_convertToResult_ReturnsDefaultErrorIfNoErrorOrResultPassedIn() {
        ...
        switch result {
        case let .failure(error as SudoEntitlementsError):
            XCTAssertEqual(error, .internalError("Invalid result returned from Entitlements Service"))
        default:
            XCTFail("Unexpected result: \(result)")
        }
    }
    ...
    func test_invalidResult_ReturnsErrorAsSudoEntitlementsErrorLocalizedDescription() {
